...
const InventoryPage = (() => {
    ...
    page.innerHTML = `
      ...
      <div id="invSubDashboard">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px" id="invStatCards">
          <div class="stat-card" id="cardTotalItem" onclick="InventoryPage.filterByStatus('')" style="cursor:pointer;transition:box-shadow .15s,transform .1s" title="Tampilkan semua item"><div class="stat-label">Total Item Aktif</div><div class="stat-value" id="statTotalItem">—</div></div>
          <div class="stat-card"><div class="stat-label">Total Unit Stok</div><div class="stat-value" id="statTotalUnit">—</div></div>
          <div class="stat-card" id="cardKosong" onclick="InventoryPage.filterByStatus('kosong')" style="cursor:pointer;transition:box-shadow .15s,transform .1s" title="Tampilkan hanya item kosong"><div class="stat-label">Item Kosong <span style="font-size:10px;color:var(--muted)">(klik)</span></div><div class="stat-value" id="statKosong" style="color:var(--danger)">—</div></div>
          <div class="stat-card" id="cardRendah" onclick="InventoryPage.filterByStatus('rendah')" style="cursor:pointer;transition:box-shadow .15s,transform .1s" title="Tampilkan hanya item stok rendah"><div class="stat-label">Stok Rendah <span style="font-size:10px;color:var(--muted)">(klik)</span></div><div class="stat-value" id="statRendah" style="color:#E8B800">—</div></div>
        </div>
  ...
  function _syncCardHighlight() {
    ['cardKosong','cardRendah'].forEach(id => {
      ...
      el.style.boxShadow = on ? '0 0 0 2px var(--primary)' : '';
