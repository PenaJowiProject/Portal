...
const InventoryPage = (() => {
  ...
  let _items       = [];
  let _kategoriList = [];
  let _activeFilter = { kategori: '', search: '', status: '' };
  ...
  function mount() {
    ...
    page.innerHTML = `
      ...
      <div id="invSubDashboard">
        <div class="page-header"><h1>Dashboard Inventory</h1><p>Statistik dan kondisi stok barang. Klik kartu untuk menyaring daftar.</p></div>
        <style>
          .inv-card-klik { cursor:pointer; position:relative; transition:box-shadow .18s, transform .12s, border-color .18s; border:1.5px solid transparent; }
          .inv-card-klik:hover { transform:translateY(-3px); box-shadow:0 6px 18px rgba(0,0,0,.10); border-color:var(--primary); }
          .inv-card-klik:active { transform:translateY(-1px); }
          .inv-card-klik .inv-card-cue { position:absolute; top:10px; right:12px; font-size:10px; font-weight:700; color:var(--primary); opacity:.55; transition:opacity .18s; }
          .inv-card-klik:hover .inv-card-cue { opacity:1; }
          .inv-card-diam { opacity:.92; }
          .inv-card-aktif { box-shadow:0 0 0 2px var(--primary), 0 6px 18px rgba(0,0,0,.10) !important; border-color:var(--primary) !important; transform:translateY(-2px); }
          .inv-card-aktif .inv-card-cue { opacity:1; }
        </style>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px" id="invStatCards">
          <div class="stat-card inv-card-klik" id="cardTotalItem" onclick="InventoryPage.filterByStatus('')" title="Klik: tampilkan semua item"><span class="inv-card-cue">lihat semua →</span><div class="stat-label">Total Item Aktif</div><div class="stat-value" id="statTotalItem">—</div></div>
          <div class="stat-card inv-card-diam"><div class="stat-label">Total Unit Stok</div><div class="stat-value" id="statTotalUnit">—</div></div>
          <div class="stat-card inv-card-klik" id="cardKosong" onclick="InventoryPage.filterByStatus('kosong')" title="Klik: tampilkan hanya item kosong"><span class="inv-card-cue">lihat →</span><div class="stat-label">Item Kosong</div><div class="stat-value" id="statKosong" style="color:var(--danger)">—</div></div>
          <div class="stat-card inv-card-klik" id="cardRendah" onclick="InventoryPage.filterByStatus('rendah')" title="Klik: tampilkan hanya stok rendah"><span class="inv-card-cue">lihat →</span><div class="stat-label">Stok Rendah</div><div class="stat-value" id="statRendah" style="color:#E8B800">—</div></div>
        </div>
  ...
  function _renderTable() {
    ...
    if (_activeFilter.search)   filtered = filtered.filter(i =>
      (String(i.nama||'')).toLowerCase().includes(_activeFilter.search) ||
      (String(i.barcode||'')).toLowerCase().includes(_activeFilter.search)
    );
    // Filter status stok — dipicu klik kartu ringkasan (Kosong/Rendah)
    // atau kartu Total (reset). Pakai stockStatus dari backend kalau ada,
    // fallback hitung sendiri biar konsisten dengan angka di kartu.
    if (_activeFilter.status === 'kosong') {
      filtered = filtered.filter(i => i.status !== 'Nonaktif' &&
        (i.stockStatus ? i.stockStatus === 'kosong' : i.totalQty === 0));
    } else if (_activeFilter.status === 'rendah') {
      filtered = filtered.filter(i => i.status !== 'Nonaktif' &&
        (i.stockStatus ? i.stockStatus === 'rendah' : (i.totalQty > 0 && i.totalQty <= (i.minThreshold||0))));
    }
    ...
    m.innerHTML = `
      <div class="modal" style="max-width:680px">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" onclick="document.getElementById('modalInvDetail').remove(); InventoryPage.openEdit('${d.id}')">✎ Edit Item</button>
        </div>
      </div>`;
  ...
  function _barcodeAutocomplete(q) {
    ...
    const matches = _items.filter(i =>
      (String(i.barcode||'')).toLowerCase().includes(q.toLowerCase()) ||
      (String(i.nama||'')).toLowerCase().includes(q.toLowerCase())
    ).slice(0, 8);
  ...
  // ── Klik kartu ringkasan → filter tabel by status stok ──
  // Klik kartu yang sama lagi = matikan filter (toggle). Kartu aktif
  // di-highlight, dan muncul banner kecil biar user sadar tabel sedang
  // difilter (bukan mengira datanya cuma segitu).
  function filterByStatus(status) {
    // Toggle: klik status yang sama → reset ke semua.
    _activeFilter.status = (_activeFilter.status === status) ? '' : status;
    _syncCardHighlight();
    _syncFilterBanner();
    _renderTable();
  }

  function _syncCardHighlight() {
    ['cardKosong','cardRendah'].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      const on = (id === 'cardKosong' && _activeFilter.status === 'kosong') ||
                 (id === 'cardRendah' && _activeFilter.status === 'rendah');
      el.classList.toggle('inv-card-aktif', on);
    });
  }

  function _syncFilterBanner() {
    let banner = document.getElementById('invFilterBanner');
    const wrap = document.getElementById('invTableBody')?.closest('.section-card');
    if (!_activeFilter.status) { if (banner) banner.remove(); return; }
    const label = _activeFilter.status === 'kosong' ? 'Item Kosong' : 'Stok Rendah';
    if (!banner && wrap) {
      banner = document.createElement('div');
      banner.id = 'invFilterBanner';
      banner.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:10px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;padding:8px 14px;margin:0 0 12px;font-size:12.5px;color:var(--primary)';
      wrap.parentNode.insertBefore(banner, wrap);
    }
    if (banner) {
      banner.innerHTML = `<span>Menampilkan hanya: <strong>${label}</strong></span>
        <button onclick="InventoryPage.filterByStatus('')" style="background:none;border:1px solid var(--primary);color:var(--primary);border-radius:6px;padding:3px 10px;font-size:12px;cursor:pointer">Tampilkan semua ✕</button>`;
    }
  }
